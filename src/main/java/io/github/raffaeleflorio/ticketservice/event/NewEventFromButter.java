  ...
  public Uni<Void> update(final Events events) {
    return this.butter
      ...
      .onItem().transform(eventData -> Json.createObjectBuilder()
        .add("origin", "BUTTER")
        .add("externalId", slug(eventData))
        .add("title", title(eventData))
        .add("description", description(eventData))
        .add("poster", poster(eventData))
        .add("date", date(eventData))
        .add("maxTickets", maxTickets(eventData))
      )
